...
function drawFrame(img, frameX, frameY, canvasY){

    ctx.drawImage(img, frameX * width, frameY * height, width, height,
                  xh, canvasY, scaleWidth, scaleHeight)
...
function heroAnimWalk(){
        ...
        fps = 0;
        ctx.clearRect(0,0, canvas.width, canvas.height);
        drawFrame(heroWalk,arrayHeroWalk[i],0,yh);
        i++;
...
function heroAnimWalkReverse(){
        ...
        fps = 0;
        ctx.clearRect(0,0, canvas.width, canvas.height);
        drawFrame(heroWalkReverse,arrayHeroWalkReverse[i],0,yh);
        i++;
...
function heroAnimAttack(){
    ...
    if(sence == 0){
        drawFrame(heroAttack,arrayHeroAttack[i],0,yh);
        i++;
        if(i >= arrayHeroAttack.length){
            i = 0;
        }
    } else if(sence == 1){
        drawFrame(heroAttackReverse,arrayHeroAttackReverse[i],0,yh);
        i++;
